...
income_pred_1 <- posterior_predict(income_model_1, newdata = test_data)

income_classifications <- test_data %>%
  mutate(income_prop = colMeans(income_pred_1)) %>%
  mutate(income_class_1 = as.numeric(income_prop >= 0.5)) %>%
  select(education_num, income_prop, income_class_1, income)

tabyl(income_classifications, income, income_class_1)
